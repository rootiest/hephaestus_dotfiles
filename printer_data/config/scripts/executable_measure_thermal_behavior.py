...
USER_ID = 'rootiest'            # e.g. Discord handle
PRINTER_MODEL = 'voron_v2_300'      # e.g. 'voron_v2_350'
HOME_TYPE = 'tap'          # e.g. 'nozzle_pin', 'microswitch_probe', etc.
PROBE_TYPE = 'tap'         # e.g. 'klicky', 'omron', 'bltouch', etc.
X_RAILS = '1x_mgn12_front'            # e.g. '1x_mgn12_front', '2x_mgn9'
BACKERS = 'none'            # e.g. 'steel_x_y', 'Ti_x-steel_y', 'mgn9_y'
NOTES = 'kinematic_bed'              # anything note-worthy about this particular run,
                        #     no "=" characters
#####################################
...
HE_TEMPERATURE = 150                # extruder temperature for measurements
MEASURE_INTERVAL = 1
N_SAMPLES = 3
...
# Extra temperature sensors to collect. Use same format as above but separate
# quoted names with commas (if more than one).
EXTRA_SENSORS = {"frame1": "temperature_sensor host",
                 "z_switch": "temperature_sensor toolboard"}
...
def write_metadata(meta):
    with open(DATA_FILENAME, 'w', encoding="utf-8") as dataout:
...
def main():
    ...
    with open(DATA_FILENAME, "w", encoding="utf-8") as out_file:
