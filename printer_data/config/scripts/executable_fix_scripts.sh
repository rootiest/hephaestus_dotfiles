...
sh_count=$(find "$path" -type f -name "*.sh" | wc -l) # Count the number of .sh files
py_count=$(find "$path" -type f -name "*.py" | wc -l) # Count the number of .py files
total_count=$((sh_count + py_count)) # Add the counts together to get the total count

# Make every .sh and .py file executable
find "$path" -type f \( -name "*.sh" -o -name "*.py" \) -print0 | while read -d $'\0' file # Find all .sh and .py files and loop through them
do
...
echo "Modified permissions for $total_count files ($sh_count .sh files and $py_count .py files) in directory $path and its subdirectories."
